...
public class SeasonFlowDao {
	 ...
	 * @param totalFlow
	 */
	public void addFlow(String phoneSeason,String phoneNumber,String upFlow,String downFlow,String totalFlow){
		Connection con=JdbcManager.getConnection();
		
		try {
			PreparedStatement ps= con.prepareStatement("insert into season_phone values(?,?,?,?,?)"
		+ "ON DUPLICATE KEY UPDATE phoneupflow=?,phonedownflow=?,phonetotalflow=?");
		ps.setString(1, phoneSeason);
